...
inline void two_means(const vector<Node*>& nodes, int f, Random& random, bool cosine, Node* p, Node* q, int iterations) {
  /*
  ...
  */
  static int iteration_steps = iterations;
...
template<typename T, typename Distance, typename Node>
inline void find_two_furthest_points_naive(const vector<Node*>& nodes, int f, bool cosine, Node* p, Node* q) {
    vector<NodeDistanceInfo> distances;
    // #5 4.359 src/annoylib.h:428:23: warning: comparison of integer expressions of different signedness: ‘int’ and ‘std::vector<Annoy::Minkowski::Node<int, float>*, std::allocator<Annoy::Minkowski::Node<int, float>*> >::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
    for (int i = 0; i < nodes.size(); i++) {
        for (int j = i + 1; j < nodes.size(); j++) {
            T dij = Distance::distance(nodes[i], nodes[j], f);
            NodeDistanceInfo currentNodeDistanceInfo = {i, j , dij};
            distances.push_back(currentNodeDistanceInfo);
        }
    }
    auto max_distance_it = std::max_element(distances.begin(), distances.end(),
                                            [](const NodeDistanceInfo& a, const NodeDistanceInfo& b) {
                                                return a.distance < b.distance;
                                            });
    if (max_distance_it != distances.end()) {
        NodeDistanceInfo max_distance = *max_distance_it;
        Distance::template copy_node<T, Node>(p, nodes[max_distance.node1], f);
        Distance::template copy_node<T, Node>(q, nodes[max_distance.node2], f);
        if (cosine) {
...
template<typename T, typename Random, typename Distance, typename Node>
inline void find_two_furthest_points_iterative(const vector<Node*>& nodes, int f, Random& random, bool cosine, Node* p, Node* q, int iterations) {
    size_t count = nodes.size();

    size_t first_random_node = random.index(count);
    size_t second_random_node = random.index(count - 1);
    second_random_node += (second_random_node >= first_random_node);

    Distance::template copy_node<T, Node>(p, nodes[first_random_node], f);
    Distance::template copy_node<T, Node>(q, nodes[second_random_node], f);

    for (int iteration = 0; iteration < iterations; iteration++) {
        size_t third_random_node = random.index(count);;
        T di = Distance::distance(p, nodes[third_random_node], f),
          dj = Distance::distance(q, nodes[third_random_node], f);
        if (di < dj) {
            // qk > pk
            Distance::template copy_node<T, Node>(p, nodes[third_random_node], f);
        } else {
            // qk < pk
            Distance::template copy_node<T, Node>(q, nodes[third_random_node], f);
        }
    }
    if (cosine) { Distance::template normalize<T, Node>(p, f); Distance::template normalize<T, Node>(q, f); }
    Distance::init_node(p, f);
...
struct Angular : Base {
  ...
  template<typename S, typename T, typename Random>
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations) {
    Node<S, T>* p = (Node<S, T>*)alloca(s);
    Node<S, T>* q = (Node<S, T>*)alloca(s);
    if (method == 0)
        two_means<T, Random, Angular, Node<S, T> >(nodes, f, random, true, p, q, iterations);
    if (method == 1)
        find_two_furthest_points_naive<T, Angular, Node<S, T> >(nodes, f,  true, p, q);
    if (method == 2)
        find_two_furthest_points_iterative<T, Random, Angular, Node<S, T> >(nodes, f, random, true, p, q, iterations);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
...
struct DotProduct : Angular {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations) {
    ...
    if (method == 0)
      two_means<T, Random, DotProduct, Node<S, T> >(nodes, f, random, true, p, q, iterations);
    if (method == 1)
      find_two_furthest_points_naive<T, DotProduct, Node<S, T> >(nodes, f,  true, p, q);
    if (method == 2)
      find_two_furthest_points_iterative<T, Random, DotProduct, Node<S, T> >(nodes, f, random, true, p, q, iterations);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
...
struct Euclidean : Minkowski {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations) {
    ...
    Node<S, T>* q = (Node<S, T>*)alloca(s);
    if (method == 0)
        two_means<T, Random, Euclidean, Node<S, T> >(nodes, f, random, false, p, q, iterations);
    if (method == 1)
        find_two_furthest_points_naive<T, Euclidean, Node<S, T> >(nodes, f,  false, p, q);
    if (method == 2)
        find_two_furthest_points_iterative<T, Random, Euclidean, Node<S, T> >(nodes, f, random, false, p, q, iterations);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
    Base::normalize<T, Node<S, T> >(n, f);
    n->a = 0.0;
...
struct Manhattan : Minkowski {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations) {
    ...
    Node<S, T>* q = (Node<S, T>*)alloca(s);
    if (method == 0)
        two_means<T, Random, Manhattan, Node<S, T> >(nodes, f, random, false, p, q, iterations);
    if (method == 1)
        find_two_furthest_points_naive<T, Manhattan, Node<S, T> >(nodes, f,  false, p, q);
    if (method == 2)
        find_two_furthest_points_iterative<T, Random, Manhattan, Node<S, T> >(nodes, f, random, false, p, q, iterations);
...
protected:
  ...
  bool _on_disk;
  bool _built;
  int _splitting_method;
  int _method_iterations;
public:
...
protected:
  ...
  S _make_tree(const vector<S>& indices, bool is_root, Random& _random, ThreadedBuildPolicy& threaded_build_policy) {
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
      ...
      D::create_split(children, _f, _s, _random, m, _splitting_method, _method_iterations);
